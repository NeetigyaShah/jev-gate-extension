...
async function generateText({ goal, field, recent }) {
  const { orKey, groqKey, textModel, textProvider } = await chrome.storage.local.get([
    'orKey', 'groqKey', 'textModel', 'textProvider',
  ]);
  const provider = textProvider === 'openrouter' ? 'openrouter' : 'groq';
  const model = textModel || (provider === 'groq' ? 'qwen/qwen3.8-27b' : 'deepseek/deepseek-v4-flash-0731');
  const key = provider === 'groq' ? groqKey : orKey;
  if (!key) return { ok: false, error: provider === 'groq' ? 'missing-groq-key' : 'missing-openrouter-key' };
  ...
  // `reasoning: {enabled:false}` is an OpenRouter control. Groq needs no such
  // flag, and its reasoning models put their output in `reasoning` while
  // `content` stays empty, which is reported rather than silently accepted.
  const body = provider === 'groq'
    ? { model, messages: [{ role: 'user', content: prompt }], max_tokens: 60 }
    : { model, reasoning: { enabled: false }, messages: [{ role: 'user', content: prompt }], max_tokens: 40 };
    ...
    if (!text) {
      return {
        ...
        error: message.reasoning
          ? `${model} spent its output on reasoning and returned no value; pick a model that answers directly`
          : 'empty-completion',
      };
    }
    if (/^none$/i.test(text)) return { ok: false, error: 'no-value-in-goal' };
    return { ok: true, text, model: data?.model || model, provider };
  } catch (e) {
    return { ok: false, error: String(e) };
  }
}
